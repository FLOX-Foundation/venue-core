 ...
 * license information.
 *
 * Decimal <-> fixed-point wire helpers for text protocols. Parsing goes
 * straight from a decimal string into the engine's fixed-point raw value with
 * NO double round-trip: exponents, signs, excess precision, overflow, empty
 * and trailing garbage are all rejected rather than silently coerced. Printing
 * emits the exact minimal decimal (no trailing zeros, no fixed "%f" padding).
 *
 * Price::Scale and Quantity::Scale are the same power of ten, so one scale
 * serves both.
 *
 * These live in the core because both ends of a text session need them and the
 * two ends have to agree byte for byte: the venue codecs read and write decimals
 * on the server side, the FIX initiator (flox/connector/fix) on the client side.
 * A single implementation is what keeps them from drifting apart.
 */
#pragma once
